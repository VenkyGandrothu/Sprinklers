...
def get_revit_version(doc):
    try:
        # Access the Revit application's version number via __revit__ (PyRevit environment)
    ...
    try:
        if geometry_instance.Symbol:
            for param in geometry_instance.Symbol.Parameters:
                output.print_md("Parameter Name: " + param.Definition.Name)
...
# New logic: Get the active view level
...
# Function to allow user to select a sprinkler family
...
# Function to ensure the selected family symbol is loaded
...
# Function to format value to 3 decimal places
        ...
        for position in positions:
            if position:
                ...
                if family_instance:
                    ...
                    # Log the placement success
                    instance_elevation = family_instance.Location.Point.Z * 304.8
                    elevation_from_level =  level_elevation_mm + instance_elevation
                else:
...
# In the InputForm class, modify the constructor to include active view name
class InputForm(Form):
    def __init__(self, doc, active_level_elevation_mm, type_name_groups, sprinkler_families):
        ...
        # Block Selection Label and Dropdown (moved to the top)
        self.label1 = Label()
        ...
        # Family Selection Label and Dropdown (moved below the block dropdown)
        self.label2 = Label()
        ...
        # Offset Value Input (moved to the last)
        self.label3 = Label()
        ...
        # Active Level Elevation and Active View Name Display
        self.label4 = Label()
    ...
    def load_blocks(self):
        ...
        display_block_names = []  # List to store the modified block names for display
        self.block_name_map = {}  # Dictionary to map display names to actual block names

        # Modify block names for display in the dropdown
        for block_name in block_names:
            display_block_name = block_name.split(".dwg.")[-1]  # Just an example to clean the name
            display_block_names.append(display_block_name)
            self.block_name_map[display_block_name] = block_name  # Mapping display name to actual block name

        # Add modified names to the dropdown
...
# Get the user-defined offset value (in mm)
desired_offset = 2000  # Default offset

# Get the selected element(s) from the user
...
if active_level:
    ...
    level_elevation_mm = level_elevation * 304.8  # Convert feet to mm
    # output.print_md("Active Level Elevation: " + str(level_elevation_mm) + " mm")
else:
...
if not selected_ids:
    ...
    for elem_id in selected_ids:
        ...
        if isinstance(element, ImportInstance):
            ...
            # output.print_md("Selected Linked CAD File ID: " + str(cad_file_id))

            import_transform = get_import_transform(element)
            # if import_transform:
            #      output.print_md("Transform obtained for ImportInstance.")

            geo_options = Options()
            geometry_element = element.get_Geometry(geo_options)

            if geometry_element:
                # output.print_md("Analyzing geometry in the selected CAD file...")
                nested_count = [0]
                for geometry_obj in geometry_element:
                    if isinstance(geometry_obj, GeometryInstance):
                        process_nested_geometry_instance(geometry_obj, output, nested_count, type_name_groups,
                                                         geometry_data=geometry_data, import_transform=import_transform)
                # output.print_md("Total Geometry Instances Processed: " + str(nested_count[0]))
                # output.print_md("Geometry data collected: " + str(len(geometry_data)))
